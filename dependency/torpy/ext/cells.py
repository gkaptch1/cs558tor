...
from dependency.torpy.ext.documents.network_status import Router
from dependency.torpy.ext.utils import AuthType, to_hex, fp_to_str
from dependency.torpy.ext.crypto import TOR_DIGEST_LEN, hybrid_encrypt
from dependency.torpy.ext.crypto_common import sha1
...
class CellRelayIntroduce1(TorCell):
    ...
    def __init__(
        self,
        introduction_point_router: Router,
        ...
        super().__init__(circuit_id)
        self.introduction_point = introduction_point_router
        self.introduction_point_key = introduction_point_key
        self.handshake_encrypted = self._create_handshake(rendezvous, rendezvous_cookie, auth_type, descriptor_cookie)

    def _create_handshake(self, introduce, rendezvous_cookie, auth_type, descriptor_cookie):
        #
        ...
        handshake += struct.pack('!4sH', socket.inet_aton(introduce.ip), introduce.or_port)
        assert len(introduce.fingerprint) == 20
        handshake += struct.pack('!20s', introduce.fingerprint)
        handshake += struct.pack('!H', len(introduce.descriptor.onion_key)) + introduce.descriptor.onion_key
