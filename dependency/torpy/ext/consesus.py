...
class DirectoryList:
    ...
    def default_authorities(cls, use_local_directories=True):
        if use_local_directories:
            ...
            nodes_list = [
                "\"test000a orport=5000 v3ident=B10694D467B0F14BD4DF5A1A9231990452C0E725 127.0.0.1:7000 B106 94D4 67B0 F14B D4DF 5A1A 9231 9904 52C0 E725\"",
                "\"test001a orport=5001 v3ident=C58DF28850BC39FE03CDE5E239C683DEFD937FB0 127.0.0.1:7001 C58D F288 50BC 39FE 03CD E5E2 39C6 83DE FD93 7FB0\"",
                "\"test002a orport=5002 v3ident=34DA7F506DE64725F4177D388DACA21DF5860D77 127.0.0.1:7002 34DA 7F50 6DE6 4725 F417 7D38 8DAC A21D F586 0D77\""
            ]
            logger.info('Getting virtual Tor network')
        else:
            logger.info('Getting live Tor network')
...
class TorConsensus:
    ...
    def get_random_guard_node(self, different_flags=None):
        ...
        if router.or_port == 5005:
            return self.get_random_guard_node(different_flags)  # try again
        logger.info(' | get_random_guard_node | ' + router.ip + ':' + str(router.dir_port) + ' AKA ' + router.nickname)
        return router

    def get_random_exit_node(self):
        flags = [RouterFlags.Fast, RouterFlags.Running, RouterFlags.Valid, RouterFlags.Exit]
        router = self.get_random_router(flags)
        logger.info(' | get_random_exit_node | ' + router.ip + ':' + str(router.dir_port) + ' AKA ' + router.nickname)
        return router

    def get_random_middle_node(self):
        flags = [RouterFlags.Fast, RouterFlags.Running, RouterFlags.Valid]
        router = self.get_random_router(flags)
        logger.info(' | get_random_middle_node | ' + router.ip + ':' + str(router.dir_port) + ' AKA ' + router.nickname)
        return router
