...
from dependency.torpy.http.base import TorInfo, SocketProxy
...
def build_tor_opener(guard=None, hops_count=3, debuglevel=0, circuit=None):
    if not guard:
        guard = circuit.guard
    if circuit:
        hops_count = len(circuit.circuit_nodes)
...
def do_request(url, method='GET', data=None, headers=None, hops=3, auth_data=None, verbose=0, retries=3):
    ...
    guard_router = tor.consensus.get_random_guard_node()
    guard = TorGuard(guard_router, purpose='TorClient', consensus=tor._consensus, auth_data=tor._auth_data)
    opener = build_tor_opener(guard, hops_count=hops, debuglevel=verbose)
