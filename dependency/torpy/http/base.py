...
from dependency.torpy.utils import hostname_key
...
class TorInfo:
    ...
    def get_circuit(self, hostname):
        ...
        with self._lock:
            ...
            if not circuit:
                try:
                    circuit = list(self._guard.circuits.values()).pop()
                except:
                    logger.error('[TorInfo] No circuits containing this particular guard (%s).', self._guard.nickname)
            # if not circuit:
            #     import telescoping_circuit
            #     try:
            #         circuit = telescoping_circuit.get_circuit_for_hostname(hostname)
            #     except:
            #         logger.error('[TorInfo] No circuit to use.  Giving up.')
            # if not circuit:
