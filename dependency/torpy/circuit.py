...
from dependency.torpy.utils import cached_property
...
from dependency.torpy.utils import ignore
from dependency.torpy.http.client import HttpStreamClient
from dependency.torpy.stream import TorStream, TorWindow, StreamsList
from dependency.torpy.crypto_state import CryptoState
from dependency.torpy.keyagreement import KeyAgreement, TapKeyAgreement, NtorKeyAgreement, FastKeyAgreement
from dependency.torpy.hiddenservice import DescriptorNotAvailable, HiddenServiceConnector
...
class CircuitNode:
    ...
    def complete_handshake(self, handshake_response, y=None, a=None):
        shared_secret = self.key_agreement.complete_handshake(handshake_response, y, a)
    ...
    def store_key(self, shared_secret):
        if self.key_agreement.TYPE == 0:
            self._crypto_state = CryptoState(shared_secret)
        if self.key_agreement.TYPE == 1:
            self._crypto_state = CryptoState(shared_secret[:KeyAgreement.KEY_MATERIAL_LENGTH])
            self._router.circuit.create(part=3)
...
class TorCircuit:
    def __init__(self, id, guard):
        ...
        self._guard._router.circuit = self
        self._guard.circuits._circuits_map[self._id] = self
    ...
    def create(self, circuit_node=None, cell_create=None, part=None):
        with self._state_lock:
            if part == 1 or not part:
                ...
                if part == 1:
                    [self._circuit_nodes, cell_created] = self._initialize(self._guard.router, circuit_node=circuit_node, cell_create=cell_create, part=part)
                    return cell_created
            if part == 2 or not part:
                self._circuit_nodes = self._initialize(self._guard.router, circuit_node=circuit_node, cell_create=cell_create)
            if part == 3 or not part:
    ...
    def _initialize(self, router, circuit_node=None, cell_create=None, part=None):
        """
        ...
        if part:
            return [[circuit_node], cell_created]

        logger.debug('Verifying response...')
        circuit_node.complete_handshake(cell_created.handshake_data, None, None)
    ...
    def send_wait(self, cell, node, wait_cell):
        return self.create(node, cell, part=1)
        return self._send_wait(cell, wait_cell)
    ...
    def extend(self, next_onion_router, key_agreement_cls=NtorKeyAgreement):
        ...
        """
        from telescoping_circuit import extend
        extend(self, next_onion_router, key_agreement_cls)
    ...
    def build_hops(self, hops_count):
        ...
        while self.nodes_count < hops_count:
            if self.nodes_count == hops_count - 1:
                router = self.guard.consensus.get_random_exit_node()
            else:
                router = self.guard.consensus.get_random_middle_node()
    ...
    def rendezvous_introduce(self, rendezvous_circuit, rendezvous_cookie, auth_type, descriptor_cookie):
        ...
        introduce = rendezvous_circuit.last_node.router
        ...
        inner_cell = CellRelayIntroduce1(
            introduction_point, public_key_bytes, introduce, rendezvous_cookie, auth_type, descriptor_cookie, self._id
        )
...
class CircuitsList:
    ...
    def initialize(self, circuit=None, circuit_node=None, cell_create=None):
        # if circuit:
        #     self._circuits_map[circuit.id] = circuit
        circuit.create(circuit_node=circuit_node, cell_create=cell_create)
