...
from dependency.torpy.crypto_common import (
    hmac,
    dh_public,
    dh_shared,
    dh_private,
...
class NtorKeyAgreement(KeyAgreement):
    ...
    @cached_property
    def handshake(self, g__b=None, g__x=None):
        g__b = g__b or self._B
        g__x = g__x or self._X
        # and generates a client-side handshake with contents:
        #   NODEID      Server identity digest  [ID_LENGTH bytes]
        #   KEYID       KEYID(B)                [H_LENGTH bytes]
        #   CLIENT_PK   X                       [G_LENGTH bytes]
        handshake = self._fingerprint_bytes
        handshake += curve25519_to_bytes(g__b)
        handshake += curve25519_to_bytes(g__x)
        return handshake

    def complete_handshake(self, handshake_response, y=None, auth=None):
        # The server's handshake reply is:
        # SERVER_PK   Y                       [G_LENGTH bytes]
        # AUTH        H(auth_input, t_mac)    [H_LENGTH bytes]
        if y and auth:
            secret_input = handshake_response
        else:
